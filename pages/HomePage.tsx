...
const HomePage: React.FC<HomePageProps> = ({ onNavigate, onQuickViewBook }) => {
  ...
  return (
    <div>
      {/* Hero Header */}
      <header className="relative pt-20 pb-32 md:pt-32 md:pb-48 bg-white dark:bg-slate-950 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <div className="inline-flex items-center gap-2 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 px-4 py-2 rounded-full mb-8 font-bold text-sm tracking-wide animate-bounce">
            <span className="material-symbols-outlined text-lg">celebration</span>
            UPDATED FOR 2024-25 SYLLABUS
          </div>
          <h1 className="text-5xl md:text-7xl font-black tracking-tight mb-8 leading-[1.1] dark:text-white">
            Learn Better, <br/>
          ...
          <div className="max-w-3xl mx-auto relative group">
            <div className="absolute -inset-2 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 rounded-[32px] blur-xl opacity-25 group-hover:opacity-60 transition duration-500"></div>
            <div className="relative bg-white dark:bg-slate-800 rounded-3xl flex items-center p-3 shadow-2xl border border-slate-100 dark:border-slate-700">
              <span className="material-symbols-outlined ml-6 text-slate-400 text-3xl">search</span>
              <input 
                className="w-full bg-transparent border-none focus:ring-0 px-6 py-4 text-xl dark:text-white placeholder:text-slate-400" 
              ...
              <button
                onClick={openLibrary}
                className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black hover:bg-indigo-700 transition-all shadow-lg hover:shadow-indigo-500/40"
              >
                Search
              </button>
            </div>
      ...
      {/* Class Focus Section */}
      <section className="py-24 relative overflow-hidden dark:bg-slate-950">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-20 relative">
            <h2 className="text-4xl md:text-5xl font-black mb-6 bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-slate-700 dark:from-white dark:to-slate-400">Primary Focus: Your Class</h2>
            ...
            <div className="absolute -top-10 left-1/2 -translate-x-1/2 opacity-10">
              <span className="material-symbols-outlined text-[120px] dark:text-slate-700">workspace_premium</span>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-center mb-16 gap-6">
            <div className="text-center md:text-left">
              ...
              <h2 className="text-4xl font-black mt-4 bg-clip-text text-transparent bg-gradient-to-r from-slate-900 to-indigo-600 dark:from-white dark:to-indigo-400">Trending Solutions</h2>
            </div>
            <button className="group bg-white dark:bg-slate-800 px-8 py-4 rounded-2xl shadow-md flex items-center gap-3 font-bold text-indigo-600 dark:text-indigo-400 hover:bg-indigo-600 hover:text-white transition-all">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {CLASS_10_BOOKS.map((book) => (
              <BookCard key={book.id} book={book} onQuickView={onQuickViewBook} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 dark:bg-slate-950">
        <div className="bg-gradient-to-br from-indigo-600 via-purple-700 to-pink-600 rounded-[40px] p-8 md:p-20 relative overflow-hidden text-white flex flex-col md:flex-row items-center gap-16 shadow-[0_40px_100px_-15px_rgba(79,70,229,0.5)]">
          ...
          <div className="md:w-3/5 relative z-10">
            <h2 className="text-4xl md:text-6xl font-black mb-8 leading-tight">Built for Your <br/><span className="text-amber-300">Success Journey</span></h2>
...
const ClassCard: React.FC<ClassCardProps> = ({ title, desc, icon, color, primaryBtn, onClick }) => {
  ...
  return (
    <div className="group relative bg-white dark:bg-slate-800 p-10 rounded-[40px] shadow-xl transition-all duration-500 hover:-translate-y-4 border border-slate-50 dark:border-slate-700 overflow-hidden flex flex-col items-center text-center">
      ...
      <div className={`w-24 h-24 bg-gradient-to-br ${colorClasses[color]} rounded-3xl flex items-center justify-center mb-10 shadow-lg transform group-hover:rotate-12 transition-transform`}>
        <span className="material-symbols-outlined text-white text-5xl">{icon}</span>
      </div>
      <h3 className="text-4xl font-black mb-4 text-slate-800 dark:text-white">{title}</h3>
      <p className="text-lg text-slate-500 dark:text-slate-400 mb-8 font-medium">{desc}</p>
      <div className="mt-auto w-full">
        <button 
          onClick={onClick}
          className={`block w-full py-5 rounded-2xl text-white font-bold text-xl transition-colors shadow-lg ${colorClasses[color].split(' ').pop()} hover:brightness-110`}
        >
...
// Fix for BookCard props to handle React key correctly
const BookCard: React.FC<{ book: Book; onQuickView?: (book: Book) => void }> = ({ book, onQuickView }) => (
  <div className="group bg-white dark:bg-slate-800 rounded-3xl overflow-hidden shadow-[0_10px_30px_-10px_rgba(0,0,0,0.1)] hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.2)] transition-all duration-500">
    <div className="aspect-[3/4] relative overflow-hidden bg-slate-200">
      <img alt={book.title} className="object-cover w-full h-full group-hover:scale-110 transition-transform duration-700" src={book.imageUrl} />
...
const FeatureItem: React.FC<FeatureItemProps> = ({ icon, title, desc, color }) => {
  ...
  return (
    <div className="flex gap-6 group">
      <div className={`bg-white/20 p-4 rounded-2xl shrink-0 group-hover:bg-white transition-all ${colors[color]}`}>
        <span className="material-symbols-outlined text-3xl">{icon}</span>
      </div>
      <div>
        <h4 className="font-black text-2xl mb-2">{title}</h4>
        <p className="text-indigo-100 text-lg opacity-90">{desc}</p>
      </div>
    </div>
  );
};
